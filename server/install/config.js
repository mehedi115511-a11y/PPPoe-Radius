...
export function renderEnv(c){return Object.entries({DATABASE_URL:databaseUrl(c),JWT_SECRET:c.jwtSecret,ROUTER_SECRET_KEY_HEX:c.routerSecretKeyHex,API_PORT:c.apiPort,CORS_ORIGIN:c.corsOrigin,ADMIN_USERNAME:c.adminUsername,BILLING_WALLET_OWNER_USER_ID:c.billingWalletOwnerUserId||"",BILLING_ADMIN_SETTLEMENT_OWNER_USER_ID:c.adminSettlementOwnerUserId||"",VPN_PUBLIC_ENDPOINT:c.vpnPublicEndpoint,VPN_WIREGUARD_PUBLIC_KEY:c.vpnWireguardPublicKey,VPN_WIREGUARD_PORT:c.vpnWireguardPort,VPN_L2TP_IPSEC_SECRET:c.vpnL2tpIpsecSecret}).map(([k,v])=>`${k}=${JSON.stringify(String(v))}`).join("\n")+"\n"}
