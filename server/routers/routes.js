...
export function registerRouterRoutes(app,db,authenticate){
 ...
 const execute=(operation,status=200)=>async(req,res,next)=>{try{const data=await operation(req);res.set('Cache-Control','no-store');if(status===204)return res.status(204).end();return res.status(status).json({data,...(Array.isArray(data)?{count:data.length}:{})})}catch(error){next(error)}};
 ...
 app.post('/api/routers/:id/assignments',authenticate,execute(req=>assignRouterRecord(db,req.auth,req.params.id,req.body)));
 app.get('/api/routers/:id/secrets',authenticate,execute(req=>getRouterSecretStatus(db,req.auth,req.params.id)));
 app.put('/api/routers/:id/secrets/:purpose',authenticate,execute(req=>{const body=req.body;if(!body||typeof body!=='object'||Array.isArray(body)||Object.keys(body).length!==1||!Object.hasOwn(body,'secret'))throw Object.assign(new Error('Only a secret value is accepted'),{status:422});return saveRouterSecret(db,req.auth,req.params.id,req.params.purpose,body.secret,process.env.ROUTER_SECRET_KEY_HEX)}));
 ...
 app.post('/api/routers/:id/health',authenticate,execute(req=>checkRouterHealth(db,req.auth,req.params.id,process.env.ROUTER_SECRET_KEY_HEX)));
