...
describe('Router/NAS HTTP routes', () => {
  ...
  it('rejects router deletion with 409 and no mutation until reference checks exist', async () => { const { app, db } = fixture(); const r = await request(app, 'DELETE', '/api/routers/7'); expect(r.status).toBe(409); expect(r.body.error).toMatch(/dependency checks/); expect(db.query.mock.calls[0][0]).toMatch(/^SELECT/); expect(db.query.mock.calls[0][1]).toEqual([7, 11]); expect(db.query).toHaveBeenCalledTimes(1); });
