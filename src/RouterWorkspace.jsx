...
export default function RouterWorkspace() {
  ...
  const [assignmentRouter, setAssignmentRouter] = useState(null), [assignmentNotice, setAssignmentNotice] = useState('');
  ...
  useEffect(() => { refresh(); }, []);
  const edit = item => { setEditing(item); setCredentialStatus(null); setDependencies(null); setAssignmentRouter(null); setAssignmentNotice(''); setForm(item ? { name: item.name, host: item.host, port: String(item.port), routerOsVersion: item.routerOsVersion, status: item.status } : empty); setError(''); };
  async function showCredentialStatus(item) {
    setBusy(true); setError(''); setCredentialStatus(null); setDependencies(null); setAssignmentRouter(null);
  ...
  async function showDependencies(item) {
    setBusy(true); setError(''); setDependencies(null); setCredentialStatus(null); setAssignmentRouter(null);
    try { const result = await routerRequest(`/api/routers/${item.id}/dependencies`); setDependencies({ ...result.data, name: item.name }); }
    catch (e) { setError(e.message); }
    finally { setBusy(false); }
  }
  async function assignRecord(routerId, body) {
    setBusy(true); setError(''); setAssignmentNotice('');
    try {
      await routerRequest(`/api/routers/${routerId}/assignments`, 'POST', body);
      setAssignmentNotice(`${body.kind === 'client' ? 'Client' : 'Package'} ${body.recordId} assigned. Verify device identity separately before enabling router removal.`);
      return true;
